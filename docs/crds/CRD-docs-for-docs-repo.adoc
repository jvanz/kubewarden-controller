...
| *`metricsPort`* __integer__ | Port used by the policy server to expose the metrics endpoint. +
When unset, defaults to 8080. Only relevant when metrics are enabled. + |  | Maximum: 65535 +
Minimum: 1 +
Optional: \{} +
