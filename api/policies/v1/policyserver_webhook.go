...
func validateSigstoreTrustConfig(ctx context.Context, k8sClient client.Client, sigstoreTrustConfig string, deploymentsNamespace string) error {
	...
	const sigstoreTrustConfigKey = "sigstore-trust-config"
	if _, ok := configMap.Data[sigstoreTrustConfigKey]; !ok {
		return fmt.Errorf("spec.SigstoreTrustConfig ConfigMap \"%s\" does not contain required key \"%s\"", sigstoreTrustConfig, sigstoreTrustConfigKey)
	}
